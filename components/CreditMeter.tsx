...
export function CreditMeter({ account }: { account: CreditAccount }) {
    ...
    <div className="soft-card p-4 sm:p-5">
      ...
      <div className="mt-5">
        <div className="flex items-end justify-between">
          <span className="text-sm text-muted">총 잔액</span>
        ...
        <div className="mt-4 grid gap-2 text-xs font-bold text-muted sm:grid-cols-2">
          <span className="rounded-lg bg-wash px-3 py-2">구독 {account.subscriptionCreditBalance.toLocaleString()}</span>
          <span className="rounded-lg bg-wash px-3 py-2">구매 {account.purchasedCreditBalance.toLocaleString()}</span>
        ...
        {account.totalCreditBalance < PACKAGE_CREDIT_COST ? (
          <p className="mt-3 rounded-lg bg-coral/10 px-3 py-2 text-sm font-bold text-coral">업로드 패키지 생성에 필요한 30 크레딧보다 부족해요.</p>
        ) : null}
      </div>
    </div>
  );
