...
function makePackage(index: number, platform: GeneratedPackage["platform"], purpose: GeneratedPackage["purpose"], style: GeneratedPackage["style"]): DemoRecord<GeneratedPackage> {
  ...
  return demo({
    id,
    title: `Demo Upload Package ${index}`,
    createdAt,
    platform,
    purpose,
    style,
    usedCredits: 0,
    captions: [
      `Sample Brand의 데모 콘텐츠 ${index}입니다. 실제 게시 전 문구와 광고 표시를 직접 확인하세요.`,
      `오늘은 ${platform}에 어울리는 짧은 업로드 패키지를 준비했어요.`,
      `가볍게 저장해두고, 업로드 직전에 톤만 맞춰보세요.`
    ],
    hashtags: ["#PostKitDemo", "#SampleBrand", "#SNS준비", "#콘텐츠패키지", "#업로드준비"],
    ctas: ["저장하고 업로드 전에 다시 확인해보세요.", "댓글로 궁금한 점을 남겨주세요."],
    hooks: ["첫 2초에 핵심만 보여주세요.", "이 문구 하나로 시선을 잡아보세요."],
    thumbnails: ["오늘의 업로드 준비", "Sample Brand Pick"],
    disclosure: "#광고 또는 #협찬 여부를 최종 게시 전 확인하세요.",
    ...
    input: {
      ...
      requiredKeywords: "Sample Brand, 데모",
    ...
    selectedCaption: `Sample Brand의 데모 콘텐츠 ${index}입니다. 실제 게시 전 문구와 광고 표시를 직접 확인하세요.`,
    liked: index === 1,
...
export function createDemoDataBundle(): DemoDataBundle {
  ...
  const brandProfile = demo<BrandProfile>({
    ...
    favoriteHashtags: "#SampleBrand #PostKitDemo",
